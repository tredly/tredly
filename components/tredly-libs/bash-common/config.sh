            ...
            case "${key}" in
                ...
                technicalOptions)
                    ...
                    if [[ -n "${strippedValue}" ]]; then
                        ...
                        local techOptionsKey="${strippedValue%%=*}"
                        local techOptionsValue="${strippedValue#*=}"

                        # validate it
                        if array_contains_substring VALID_TECHNICAL_OPTIONS[@] "^${techOptionsKey}$"; then
                            _CONF_TREDLYFILE_TECHOPTIONS[${techOptionsKey}]="${techOptionsValue}"
                        fi
                    fi
                    ;;
                # startup commands
